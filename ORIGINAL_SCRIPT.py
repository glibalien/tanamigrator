#!/usr/bin/env python3
"""
Tana to Obsidian Migration Script
...
class TanaToObsidian:
    def __init__(self, json_path: str, output_dir: str):
        ...
        # Field IDs for meeting supertag
        self.project_field_id = 'zaD_EkMhKP'  # Project field in #meeting
        self.people_involved_field_id = 'znaT5AHKXQkR'  # People Involved field in #meeting
        # Field IDs for person supertag
        self.company_field_id = 'LNd_B370Hr'  # Company field in #person
        # Field IDs for recipe supertag
        self.cookbook_field_id = 'Y_572Dtvp_QO'  # Cookbook field in #recipe
        self.url_field_id = 'SYS_A78'  # URL field (Tana system field)
    ...
    def build_indices(self):
        ...
        print(f"Found {len(self.supertags)} supertags")
        if self.task_tag_id:
            print(f"  Task tag ID: {self.task_tag_id}")
        if self.day_tag_id:
            print(f"  Day tag ID: {self.day_tag_id}")
        if self.week_tag_id:
            print(f"  Week tag ID: {self.week_tag_id} (will be skipped)")
        if self.year_tag_id:
            print(f"  Year tag ID: {self.year_tag_id} (will be skipped)")
        if self.readwise_tag_id:
            print(f"  Readwise tag ID: {self.readwise_tag_id} (will be skipped)")
        if self.meeting_tag_id:
            print(f"  Meeting tag ID: {self.meeting_tag_id}")
        if self.note_tag_id:
            print(f"  Note tag ID: {self.note_tag_id}")
        if self.project_tag_id:
            print(f"  Project tag ID: {self.project_tag_id}")
        if self.person_tag_id:
            print(f"  Person tag ID: {self.person_tag_id}")
        if self.one_on_one_tag_id:
            print(f"  1:1 tag ID: {self.one_on_one_tag_id}")
        if self.recipe_tag_id:
            print(f"  Recipe tag ID: {self.recipe_tag_id}")
        if self.highlight_tag_ids:
            print(f"  Highlight tag IDs: {self.highlight_tag_ids} (will be skipped)")
        if self.field_definition_tag_ids:
            print(f"  Field-definition tag IDs: {self.field_definition_tag_ids} (will be skipped)")
        ...
        print(f"Built metanode->tags map with {len(self.metanode_tags)} entries")

        # Build meeting -> project/people indices via tuples
        # Tuples link meetings to their field values: tuple._ownerId = meeting,
        # tuple.children = [field_id, value_node_id]
        for doc in self.docs:
            if doc.get('props', {}).get('_docType') == 'tuple':
                children = doc.get('children', [])
                owner_id = doc.get('props', {}).get('_ownerId')
        ...
        print(f"Built meeting->projects map with {len(self.meeting_projects)} entries")
        print(f"Built meeting->people map with {len(self.meeting_people)} entries")
        print(f"Built person->companies map with {len(self.person_companies)} entries")
        print(f"Built recipe->cookbooks map with {len(self.recipe_cookbooks)} entries")
        print(f"Built node->urls map with {len(self.node_urls)} entries")
        ...
        print(f"Found {len(self.image_urls)} image URLs")

        # Build image metadata -> URL mapping for nodes with _imageWidth
        # These nodes store their URL through metanode -> tuple -> URL structure
        ...
        print(f"Mapped {len(self.image_metadata_urls)} image metadata nodes to URLs")
    ...
    def extract_filename_from_url(self, url: str) -> str:
        ...
        # Pattern: UUID-UUID-actualfilename.ext
        # Try to extract just the actual filename
        parts = filename.split('-')

        # Look for the part that starts with letters (not UUID) or ends with extension
        for i, part in enumerate(parts):
            # UUID parts are typically 8-4-4-4-12 hex characters
            # If this part starts with a letter and isn't all hex, it's probably the start of the filename
            if re.match(r'^[A-Za-z]', part) and not re.match(r'^[0-9A-Fa-f]+$', part):
                filename = '-'.join(parts[i:])
                break
            # Or if it contains the file extension
    ...
    def convert_references(self, text: str, download_images: bool = True) -> str:
    ...
    def get_node_modified_timestamp(self, doc: dict) -> str:
        ...
        try:
            if isinstance(modified_ts, list):
                ...
                parsed = json_module.loads(modified_ts)
    ...
    def write_merged_files(self):
        ...
        for filename, entries in self.pending_merges.items():
    ...
    def should_skip_doc(self, doc: dict) -> bool:
        ...
        # Skip nodes tagged with #week or #year
        if self.week_tag_id and self.has_tag(doc, self.week_tag_id):
            return True
        if self.year_tag_id and self.has_tag(doc, self.year_tag_id):
            return True

        # Skip nodes tagged with #highlight or #field-definition
        ...
        # Skip Readwise integration nodes
        ...
        # These are nodes with names like "!" or exclamation marks with numbers
    ...
    def should_skip_referenced_node(self, doc: dict) -> bool:
        ...
        # Skip nodes tagged with skippable tags
        if self.week_tag_id and self.has_tag(doc, self.week_tag_id):
            return True
        if self.year_tag_id and self.has_tag(doc, self.year_tag_id):
            return True
    ...
    def export_tagged_node(self, doc: dict, filename: str, folder: Path, daily_date: str = None) -> str:
        ...
        # If no daily_date provided, fall back to the node's created date
    ...
    def run(self):
        ...
        print("\nPhase 1: Exporting daily notes...")

        # First pass: export daily notes and collect tagged nodes
        for doc in self.docs:
            ...
            if self.is_daily_note(doc):
                ...
                if tagged_nodes is None:
                    ...
                    if daily_count % 100 == 0:
                        print(f"  Exported {daily_count} daily notes...")

        print(f"\nPhase 2: Collecting {len(all_tagged_nodes)} tagged nodes for merging...")

        # Second pass: collect tagged nodes for merging
        for node, filename, daily_date in all_tagged_nodes:
            ...
            if tagged_count % 100 == 0:
                print(f"  Collected {tagged_count} tagged nodes...")

        # Third pass: find orphan tagged nodes (not under any daily note)
        print("\nPhase 3: Collecting orphan tagged nodes...")
        orphan_count = 0
        ...
        # Fourth pass: write all merged files
        print(f"\nPhase 4: Writing {len(self.pending_merges)} merged files...")
        single_count, merged_count = self.write_merged_files()

        # Fifth pass: create files for referenced nodes that don't have files yet
        print(f"\nPhase 5: Creating files for referenced nodes...")
        referenced_count = 0
        for node_id in self.referenced_nodes:
            ...
            # But don't skip based on lack of supertag - that's the point
        ...
        print(f"  Created {referenced_count} files for referenced nodes")

        print("\n=== Export Complete ===")
        print(f"Daily notes: {daily_count} → Daily Notes/ ({blank_daily_count} blank skipped)")
        print(f"Tagged nodes (from daily notes): {tagged_count}")
        print(f"Orphan tagged nodes: {orphan_count}")
        print(f"Total tagged nodes: {tagged_count + orphan_count}")
        print(f"Referenced nodes: {referenced_count}")
        print(f"Files written: {len(self.pending_merges) + referenced_count} ({single_count} single, {merged_count} merged, {referenced_count} referenced)")
        print(f"Skipped: {skipped_count}")
        print(f"\nImages downloaded: {len(self.downloaded_images)} → Attachments/")
        if self.image_download_errors:
            print(f"Image download errors: {len(self.image_download_errors)}")
...
if __name__ == '__main__':
    ...
    json_path = '/home/barry/Projects/tanaexporter/tanalibrary.json'
    output_dir = '/home/barry/Projects/tanaexporter/ObsidianOutput'

    converter = TanaToObsidian(json_path, output_dir)
    converter.run()
